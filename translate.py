...
if __name__ == '__main__':
    sys.stdout = codecs.getwriter("utf-8")(sys.stdout.detach()) #codecs.getwriter(인코딩) : 주어진 인코딩의 코덱을 찾아서 반환
    ...
    src_vocab, tgt_vocab, is_reverse = get_vocabs(train_config, config, saved_data) #is_reverse : False
    ...
    with torch.no_grad():
        ...
        for lines in read_text(batch_size=config.batch_size):
            # Since packed_sequence must be sorted by decreasing order of length,packed_sequence는 내림차순
            ...
            sorted_tuples = sorted(
                ...
                key=itemgetter(1),#길이가 긴게 위로 가도록
                reverse=True,
            )
