...
class MaximumLikelihoodEstimationEngine(Engine):#이것은 teacher-forcing으로 학습하는 방식 engine
    ...
    def train(engine, mini_batch):
        ...
        engine.model.train()
        #Below this, this is Gradient Accumulation
        if engine.state.iteration % engine.config.iteration_per_update == 1 or engine.config.iteration_per_update == 1:#매번 업데이트 한다는뜻
            if engine.state.iteration > 1:
                engine.optimizer.zero_grad()
 
        device = next(engine.model.parameters()).device
        mini_batch.src = (mini_batch.src[0].to(device), mini_batch.src[1])#torchtext가 제공하는 형태로 들어있다.mini_batch.src[0]에는 tensor, mini_batch.src[1]에는 length
        mini_batch.tgt = (mini_batch.tgt [0].to(device), mini_batch.tgt[1])#decoder에 들어가는 target sentence

        # Raw target variable has both BOS and EOS token. 
        # The output of sequence-to-sequence does not have BOS token. 
        # Thus, remove BOS token for reference.
        x, y = mini_batch.src, mini_batch.tgt[0][:, 1:]#bos를 뺀 것,tgt는 length값(mini_batch.tgt[1]은빼고
        # |x| = (batch_size, length)
        # |y| = (batch_size, length)
        ...
        with autocast(not engine.config.off_autocast):
            ...
            y_hat = engine.model(x, mini_batch.tgt[0][:, :-1])#eos를 뺀 것,이건 모델에 넣어주는것
            # |y_hat| = (batch_size, length, output_size)

            loss = engine.crit(
                y_hat.contiguous().view(-1, y_hat.size(-1)),#(bs*length,output_size)
                y.contiguous().view(-1)#(bs*length)
            )
            backward_target = loss.div(y.size(0)).div(engine.config.iteration_per_update) #back-propagation을 수행할 loss는 이것
        ...
        if engine.state.iteration % engine.config.iteration_per_update == 0 and engine.state.iteration > 0: #이 조건문을 만족안하면 back-propagation만하고 gradient descent를 수행하지 않는다.
    ...
    def attach(
        ...
        if verbose >= VERBOSE_EPOCH_WISE:#1epoch가 끝날때마다
            @train_engine.on(Events.EPOCH_COMPLETED)#train engine의 epoch가 끝날때마다 이거를 실행해!
...
class SingleTrainer():#train engine과 validation engine을 동시에 물고있는 class

    def __init__(self, target_engine_class, config):
        self.target_engine_class = target_engine_class#여기에 MaximumLikelihoodEstimationEngine class를 넣어준다.
        self.config = config
    ...
    def train(
        ...
        train_engine = self.target_engine_class(
            self.target_engine_class.train,#MaximumLikelihoodEstimationEngine class의 @staticmethhod train
            model,
            crit,#loss function
            optimizer,
            lr_scheduler,
        ...
        validation_engine = self.target_engine_class(
            self.target_engine_class.validate,#MaximumLikelihoodEstimationEngine class의@staticmethod validate
            model,
            crit,
            optimizer=None,
            lr_scheduler=None,
        ...
        def run_validation(engine, validation_engine, valid_loader):
            validation_engine.run(valid_loader, max_epochs=1)#max_epochs=1인이유, train-epoch가 끝날때마다 실행되므로
        ...
        train_engine.add_event_handler(
            Events.STARTED,#애초에 처음시작할때(0epoch)
