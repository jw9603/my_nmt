...
class Attention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, dk=64):
        # |Q| = (batch_size, m, hidden_size)  -> 실제로는 (n_splits x bs, m, hs/n_splits)
        # |K| = |V| = (batch_size, n, hidden_size)
        # |mask| = (batch_size, m, n) -> 실제로는 (n_splits x bs, m, n)
        ...
        c = torch.bmm(w, V)
        # |c| = (batch_size, m, hidden_size) -> 실제로는 (n_splits x bs, m, hs/n_splits)
...
class MultiHead(nn.Module):
    ...
    def forward(self,Q, K, V, mask = None):
        # Multi-head attention은 두 가지 : encoder, decoder에 있는 self-attention, 이 경우엔 q,k,v모두 n(enc)or m(dec),encoder-decoder self attention인 경우는 다음 모양과 같음  
        # |Q|    = (batch_size, m, hidden_size)
        # |K|    = (batch_size, n, hidden_size)
        # |V|    = |K|
        # |mask| = (batch_size, m, n), this is encoder-decoder attention
        ...
        # 위의 것들은 리스트
        # |QW_i| = (bs, m, hidden_size/n_splits)
        # |KW_i| = |VW_i| = (bs, n, hidden_size/n_splits)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, key_and_value, mask, prev, future_mask):
        # |key_and_value| = (batch_size, n, hidden_size) -> key & value는 encoder에서 온 것
        # |mask|          = (batch_size, m, n)

        if prev is None : # Training mode
            # |x|           = (batch_size, m, hidden_size)
            # |prev|        = None
            # |future_mask| = (batch_size, m, m) -> self-attention을 할 때 미래 못보게 하는 mask
...
class Transformer(nn.Module):
    ...
    def _generate_pos_enc(self,hidden_size,max_length):
        ...
        dim = torch.arange(0, hidden_size // 2).unsqueeze(0).float() # (1, hs//2)
    ...
    def batch_beam_search(
        ...
        while sum(done_cnt) < batch_size and length <= max_length:
            ...
            # |fab_z|        = (current_batch_size, n, hidden_size) -> encoder's output
