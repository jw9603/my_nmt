...
class Attention(nn.Module):
    ...
    def forward(self,h_src, h_t_tgt, mask=None):
        ...
        weight = self.softmax(weight) #이게 softmax(qk^T)
...
class Decoder(nn.Module):
    ...
    def __init__(self,word_vec_size, hidden_size, n_layers=4,dropout_p=.2):
        ...
        self.rnn = nn.LSTM(
            word_vec_size+hidden_size,#input feeding
            hidden_size,
            num_layers=n_layers,
            dropout=dropout_p,
            bidirectional=False,
    ...
    def forward(self,emb_t,h_t_1_tilde,h_t_1):
        ...
        if h_t_1_tilde is None:
            #first step
            h_t_1_tilde = emb_t.new(batch_size,1,hidden_size).zero_()#emb_t와 같은 device, 같은 size 로 만들어줘

        #input feeding trick.
        x = torch.cat([emb_t,h_t_1_tilde],dim=-1)#(bs,1, ws+hs)

        y, h = self.rnn(x,h_t_1)

        return y,h  #|y| = (bs,1,hs), |h[0]|=|h[1]| = (num_layers,bs,hs)
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        #|x| = (batch_size, length, hidden_size) #학습시에는 이전  time-step의 x가 필욯지 않으므로 한번에 다 넣는다.
...
class Seq2Seq(nn.Module):
    ...
    def __init__(
        ...
        self.concat = nn.Linear(hidden_size * 2, hidden_size) #attention에서 나온 context_vector와 decoder의 hidden_state를 concat한것을 Linear에 넣는다.h_tilde를 만들기 위해
        self.tanh = nn.Tanh()
    ...
    def merge_encoder_hiddens(self, encoder_hiddens):
        ...
        for i in range(0, hiddens.size(0), 2):
            new_hiddens += [torch.cat([hiddens[i], hiddens[i + 1]], dim=-1)]#(1,bs,hs/2)x2 ==> (bs,hs)
            new_cells += [torch.cat([cells[i], cells[i + 1]], dim=-1)]

        new_hiddens, new_cells = torch.stack(new_hiddens), torch.stack(new_cells)#stack을 하므로 (#layers,bs,hs)
    ...
    def fast_merge_encoder_hiddens(self, encoder_hiddens):
        ...
        #|h_0_tgt| = (#layersx2,bs,hs/2)
        #| h_0_tgt.transpose(0, 1)| = (bs,#layersx2,hs/2) --> view ->(bs,#layers,hs)
    ...
    def forward(self, src, tgt):
        #|src| = (bs,n,|V_src|) ==> (bs,n) ,왜냐하면 원핫벡터로 들어가서 인덱스 값만들어가므로 (bs,n)
        #|tgt| = (bs,m,|v_tgt|) ==> (bs,m), tgt는 decoder의 인풋
        #|output| = (bs,m,|v_tgt|)
        ...
        h_src, h_0_tgt = self.encoder((emb_src, x_length))#input looks like tuple.
        # |h_src| = (batch_size, length, hidden_size)
        # |h_0_tgt| = (n_layers * 2, batch_size, hidden_size / 2) -->encoder의 마지막 time-step의 전체 hidden state

        h_0_tgt = self.fast_merge_encoder_hiddens(h_0_tgt)#h_0_tgt에는 hidden state와 cell state가 들어있음
        emb_tgt = self.emb_dec(tgt)#teacher forcing
        ...
        for t in range(tgt.size(1)):#target의 길이만큼
            # Teacher Forcing: take each input from training set,
            
            emb_t = emb_tgt[:, t, :].unsqueeze(1)#input feeding을 해야하므로 1 time-step씩 들어가야함,inference는 당연한거고 
            #|emb_t[:,t,:]|=(bs,ws)
            # |emb_t| = (batch_size, 1, word_vec_size)
            ...
            # |decoder_hidden| = (n_layers, batch_size, hidden_size), 얘는 튜플:hidden_state와 cell_state로 이루어짐
    ...
    def search(self,src,is_greedy=True,max_length=255): #is_greedy가 false이면 random sampling
        ...
        is_decoding = x.new_ones(batch_size,1).bool() #flag
        h_t_tilde,y_hats,indice = None,[],[]
        
        while is_decoding.sum()>0 and len(indice) < max_length:

            emb_t = self.emb_dec(y)#|emb_t| = (bs,1,ws)
            ...
            #|y_hat| = (bs,1,output_size)
            y_hats += [y_hat]

            if is_greedy:
                y = y_hat.argmax(dim=-1) #|y| = (bs,1)
            else:
                y = torch.multinomial(y_hat.exp().view(batch_size,-1),1)#|y| = (bs,1)
        ...
        #|y_hats| = (bs,length,output_size)
        #|indice| = (bs,length) -->output마다 해당하는 값의 인덱스가 들어있을것
    ...
    def batch_beam_search(
        ...
        #initialize 'SingleBeamSearchBoard' as many as batch size
        boards = [SingleBeamSearchBoard(
            h_src.device,#device
            { #prev_status_config
                'hidden_state' : {
                    'init_status' : h_0_tgt[0][:,i,:].unsqueeze(1),
                    'batch_dim_index' : 1,#hidden_state에 대한 config
                }, #|hidden_state| = (n_layers,batch_size,hs)
                'cell_state' : {
                    'init_status' : h_0_tgt[1][:,i,:].unsqueeze(1),
                    'batch_dim_index' : 1,#cell state에 대한 config
                },#|cell_state| = (n_layers,bs,hs)
                'h_t_1_tilde' : {
                    'init_status' : None,#decoder 처음에는 None임
                    'batch_dim_index' : 0
                }#|h_t_1_tilde| = (bs,1,hs)
            },
            beam_size = beam_size,
        ...
        while sum(is_done) < batch_size and length <= max_length:
            ...
            #|fab_decoder_output| = (current_bsatch_size,1,hidden_size)
            context_vector = self.attn(fab_h_src,fab_decoder_output,fab_mask)
            #|context_vector| = (current_bs,1,hs)
            fab_h_t_tilde = self.tanh(self.concat(torch.cat([fab_decoder_output,
                                                            context_vector
                                                            ],dim=-1)))
             # |fab_h_t_tilde| = (current_batch_size, 1, hidden_size)
            y_hat = self.generator(fab_h_t_tilde)
            #|y_hat| = (current_bs,1,output_size)

            #seperate the result for each sample.
            #fab_hidden[:,begin:end,:] = (n_layers,beam_size,hs)
            # fab_cell[:,begin:end,:] = (n_layers,beam_size,hs)
            # fab_h_t_tilde[begin:end] = (beam_size,1,hs) 
            ...
            for board in boards:
                if board.is_done() == 0:
                    ...
                    #pic k-best results for each sample.
