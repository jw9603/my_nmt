...
class SingleBeamSearchBoard():
    ...
    def __init__(
        self,
        device,
        prev_status_config,#중요!!,input,last_hidden_state,last_h_tilde
        beam_size=5,
        max_length=255,
    ):
        ...
        # Cumulative log-probability for each beam. 누적 확률
        self.cumulative_probs = [torch.FloatTensor([.0] + [-float('inf')] * (beam_size - 1)).to(self.device)]
        #로그 확률이므로 1은 0, 0은 -무한대가 됨 
        ...
        for prev_status_name, each_config in prev_status_config.items():
            #keys:prev_Status_name, value : each_config
            init_status = each_config['init_status'] #|init_status|=(n_layers,1,hs) or (bs=1,1,hs)
            batch_dim_index = each_config['batch_dim_index']
            if init_status is not None:
                self.prev_status[prev_status_name] = torch.cat([init_status] * beam_size,
                                                               dim=batch_dim_index)
                #|prev_status[prev_status_name]| = (n_layers,beam_size,hidden_size) or (beam_size,1,hs)
            else:
    ...
    def collect_result(self, y_hat, prev_status):
        ...
        cumulative_prob = self.cumulative_probs[-1].masked_fill_(self.masks[-1], -float('inf'))#mask는 끝나면 1 안끝나면 0,이것은 끝나면 -무한대를 넣는다는 말
        ...
        self.beam_indice += [top_indice.div(float(output_size)).long()] #int 보다 범위가 큰 정수
        ...
        for prev_status_name, prev_status in prev_status.items():
            self.prev_status[prev_status_name] = torch.index_select(
                ...
                index=self.beam_indice[-1] # ex) beam_indice[-1] = [2,2,1]
            ).contiguous()  #ex)  self.prev_status[prev_status_name] = torch.cat([prev_status[:,2,:],prev_status[:,2,:],prev_status[:,1,:]],dim=1)
