...
# from nmt.models.rnnlm import LanguageModel
...
def get_crit(output_size, pad_index):#다음 단어를 예측하는것이므로 classification으로 볼 수 있다.
    # Default weight for loss equals to 1, but we don't need to get loss for PAD token.
    # Thus, set a weight for PAD to zero. output의 pad값은 점수를 주고 싶지 않다.
    loss_weight = torch.ones(output_size)
    loss_weight[pad_index] = 0. #pad를 맞추는 경우=0
...
def get_scheduler(optimizer, config):#sgd를 쓸 경우 사용
    if config.lr_step > 0:
...
def main(config, model_weight=None, opt_weight=None):
    def print_config(config):
        ...
        pp.pprint(vars(config))#vars(object) --> 객체의 __dict__ 속성 반환, _dict__란? 객체의 변경 가능한 속성을 포함하는 사전
    ...
    loader = DataLoader(
        ...
        (config.lang[:2], config.lang[-2:]),    # Source and target language.,--lang : enko ->[:2]=en, [-2:] = ko
    ...
    if model_weight is not None: # continue_train.py 할 때 사용
